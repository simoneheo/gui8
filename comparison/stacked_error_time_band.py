...
class StackedErrorTimeBandComparison(BaseComparison):
    ...
    def generate_plot(self, fig: Optional[plt.Figure] = None, 
        ...
        try:
            ...
            # Get overlay options
            show_legend = self.kwargs.get('show_legend', True)
            show_total_line = self.kwargs.get('show_total_line', False)
            color_scheme = self.kwargs.get('color_scheme', 'viridis')
            transparency = self.kwargs.get('transparency', 0.7)
            smooth_bands = self.kwargs.get('smooth_bands', False)
