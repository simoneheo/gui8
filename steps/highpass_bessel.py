...
class highpass_bessel_step(BaseStep):
    ...
    category = "Filter"
    description = "Apply highpass Bessel filter to preserve signal shape while removing low frequencies below the cutoff."
    tags = ["time-series", "filter", "highpass", "scipy", "bessel", "frequency", "cutoff"]
    params = [
    ...
    def _validate_filter_design(cls, params: dict, y: np.ndarray) -> None:
        ...
        try:
            b, a = bessel(N=order, Wn=normal_cutoff, btype='high', analog=False, norm='phase')
        except ValueError as e:
            raise ValueError(f"Bessel highpass filter design failed: {str(e)}")
        
        # Check if signal is long enough for the filter
        padlen = 3 * max(len(a), len(b))
        if len(y) <= padlen:
            raise ValueError(
                f"Signal too short for Bessel highpass filter: "
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        ...
        try:
            ...
            return cls.create_new_channel(
                ...
                ydata=y_final,
                params=params,
                suffix="HighpassBessel"
            )
        except Exception as e:
            if isinstance(e, ValueError):
                raise e
            else:
                raise ValueError(f"Bessel highpass filter processing failed: {str(e)}")
