...
class lowpass_bessel_step(BaseStep):
    name = "lowpass_bessel"
    category = "Filter"
    description = "Apply lowpass Bessel filter to preserve signal shape while removing high frequencies above the cutoff."
    tags = ["time-series", "filter", "lowpass", "scipy", "bessel", "frequency", "cutoff"]
    params = [
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        ...
        try:
            ...
            return cls.create_new_channel(
                ...
                ydata=y_final,
                params=params,
                suffix="LowpassBessel"
            )
        except Exception as e:
            if isinstance(e, ValueError):
                raise e
            else:
                raise ValueError(f"Bessel lowpass filter processing failed: {str(e)}")
