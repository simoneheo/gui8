...
@register_step
class ppg_nn_intervals_step(BaseStep):
    name = "ppg_nn_intervals"
    category = "HeartPy"
    description = """Extract NN intervals from raw PPG using HeartPy processing.
    
This step extracts normal-to-normal (NN) intervals from PPG signals:
• **Peak detection**: Identifies heart beats from PPG signal
• **NN interval calculation**: Computes time between consecutive normal beats
• **Artifact removal**: Filters out ectopic beats and artifacts
• **Time series output**: Generates NN interval time series

• **Sampling rate**: Signal sampling frequency (automatically detected from channel)
• **Output**: NN interval time series (sample indices as x-axis)
...
• **Cardiac research**: Analyze heart rate dynamics"""
    ...
    tags = ["ppg", "nn", "heartpy", "intervals", "cardiac", "rhythm", "beats","time-series"]
    ...
    def _validate_input_data(cls, y: np.ndarray) -> None:
        ...
        if len(y) < 1000:
            raise ValueError("PPG signal too short for NN interval extraction (minimum 1000 samples)")
    ...
    def _validate_parameters(cls, params: dict) -> None:
        ...
        if fs is not None and fs < 10:
            raise ValueError("Sampling frequency too low for PPG NN interval extraction (minimum 10 Hz)")

    @classmethod
    def _validate_output_data(cls, y_original: np.ndarray, y_new: np.ndarray) -> None:
        """Validate output NN intervals"""
        if len(y_new) == 0:
            raise ValueError("NN interval extraction produced empty result")
        if np.any(np.isnan(y_new)):
            raise ValueError("NN intervals contain NaN values")
        if np.any(y_new <= 0):
            raise ValueError("NN intervals contain non-positive values")
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        """Apply PPG NN interval extraction to the channel data"""
        try:
            x = channel.xdata
            y = channel.ydata
