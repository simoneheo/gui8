...
@register_step
class highpass_butter_step(BaseStep):
    name = "highpass_butter"
    category = "Filter"
    description = "Apply highpass Butterworth filter with flat frequency response to remove low frequencies below the cutoff."
    tags = ["time-series", "filter", "highpass", "scipy", "butter", "butterworth", "frequency", "cutoff"]
    params = [
    ...
    def _validate_filter_design(cls, params: dict, y: np.ndarray) -> None:
        ...
        try:
            ...
            raise ValueError(f"Butterworth highpass filter design failed: {str(e)}")
        
        # Check if signal is long enough for the filter
        padlen = 3 * max(len(a), len(b))
        if len(y) <= padlen:
            raise ValueError(
                f"Signal too short for Butterworth highpass filter: "
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        ...
        try:
            ...
            return cls.create_new_channel(
                ...
                ydata=y_final,
                params=params,
                suffix="HighpassButterworth"
            )
        except Exception as e:
            if isinstance(e, ValueError):
                raise e
            else:
                raise ValueError(f"Butterworth highpass filter processing failed: {str(e)}")
