...
@register_step
class random_forest_feature_importance_step(BaseStep):
    name = "random_forest_feature_importance"
    category = "Feature Selection"
    description = "Use Random Forest to extract feature importance scores"
    tags = ["sklearn", "feature selection"]
    params = []

    @classmethod
    def get_info(cls): 
        return f"{cls.name} — Random Forest Feature Importance (Category: {cls.category})"
    ...
    def _validate_metadata(cls, channel: Channel) -> None:
        ...
        if features is None:
            raise ValueError("Channel metadata must contain 'features' for Random Forest analysis")
        if labels is None:
            raise ValueError("Channel metadata must contain 'labels' for Random Forest analysis")
        ...
        if features.shape[1] < 2:
            raise ValueError("At least 2 features required for Random Forest analysis")
    ...
    def _validate_output_data(cls, y_original: np.ndarray, y_new: np.ndarray) -> None:
        ...
        if len(y_new) == 0:
            raise ValueError("Random Forest produced empty feature importance scores")
        if np.any(np.isnan(y_new)):
            raise ValueError("Random Forest produced NaN feature importance scores")
        if np.any(np.isinf(y_new)):
            raise ValueError("Random Forest produced infinite feature importance scores")
        if np.all(y_new == 0):
            raise ValueError("Random Forest produced all zero feature importance scores")
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        ...
        try:
            ...
            return cls.create_new_channel(
                ...
                ydata=y_new,
                params=params,
                suffix="RFImportances"
            )
        except Exception as e:
            if isinstance(e, ValueError):
                raise e
            else:
                raise ValueError(f"Random Forest feature importance processing failed: {str(e)}")
    ...
    def script(cls, x: np.ndarray, y: np.ndarray, channel: Channel, params: dict) -> tuple[np.ndarray, np.ndarray]:
        ...
        # Create feature index array
        x_new = np.arange(len(importances))
