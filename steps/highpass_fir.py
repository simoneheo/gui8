...
def highpass_fir(y, fs, cutoff=0.5, numtaps=101, window='hamming'):
    ...
    if numtaps % 2 == 0:
        raise ValueError(f"Number of taps must be odd for highpass FIR filter, got {numtaps}")
    if fs <= 0:
...
class highpass_fir_step(BaseStep):
    name = "highpass_fir"
    category = "Filter"
    description = "Applies a high-pass FIR filter using a window method."
    tags = ["time-series"]
    params = [
    ...
    def apply(cls, channel: Channel, params: dict) -> Channel:
        ...
        params = cls._inject_fs_if_needed(channel, params, highpass_fir)
        y_new = highpass_fir(y, **params)
