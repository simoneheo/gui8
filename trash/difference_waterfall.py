...
class DifferenceWaterfallComparison(BaseComparison):
    ...
    def generate_plot(self, ax, ref_data: np.ndarray, test_data: np.ndarray, 
        ...
        try:
            ...
            color_by_sign = plot_config.get('color_by_sign', self.kwargs.get('color_by_sign', True))
            show_zero_line = plot_config.get('show_zero_line', self.kwargs.get('show_zero_line', True))
            show_trend_line = plot_config.get('show_trend_line', self.kwargs.get('show_trend_line', False))
            bar_width = plot_config.get('bar_width', self.kwargs.get('bar_width', 0.8))
